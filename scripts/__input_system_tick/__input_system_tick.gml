...
function __input_system_tick()
{
    ...
    if (INPUT_ON_PC || INPUT_ON_WEB)
    {
        ...
        else
        {
                 ...
                 ||  (__INPUT_ON_MACOS && !INPUT_ON_WEB && _global.__pointer_moved))
            {
                ...
                //Retrigger mouse capture timer to avoid the mouse runing all over the place when we refocus the window
    ...
    if (__INPUT_ON_WINDOWS)
    {
        //number clicks from touchpad and touchscreen taps (system-setting dependent)
